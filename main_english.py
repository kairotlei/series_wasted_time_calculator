...
wasted_time_hrs = round(float((mins_per_ch * chapters * seasons) / 60), 2)
save_previous_mins = round(float(((previously_duration * chapters * seasons) / 60)), 2)
save_intro_mins = round(float(((intro_duration * chapters * seasons) / 60)), 2)
save_outro_mins = round(float(((credits_length * chapters * seasons) / 60)), 2)
...
print(f"""
Congratulations, watching '{series_name}' completely you would waste {wasted_time_hrs} hours of your life, distributed in {seasons} seasons with a total of {total_cha} chapters!
...
So, in conclusion:
- Watching '{series_name}' would cost you {wasted_time_hrs} irrecoverable hours of you life.
...
webbrowser.open(f"""https://twitter.com/intent/tweet?text=Hey, if I completely watch the '{series_name}' series, I would waste {wasted_time_hrs} hours of my life into it. Ignoring flourishes I could recover {total_savings} minutes! And I also wasted {time_final} seconds while discovering this!!""")
